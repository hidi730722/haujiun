...
async function loadLogo() {
  ...
  const emptyLabel = document.getElementById('logoEmptyLabel');
...
function bulletRowHtml(i) {
  const options = Object.entries(ICON_LABELS)
    .map(([k, label]) => `<option value="${k}">${label}</option>`)
    .join('');
  return `<div class="bullet-row">
    <select data-bullet-icon="${i}">${options}</select>
    <input type="text" data-bullet-text="${i}" placeholder="重點說明（例如：晶透清晰・光澤通透）" />
  </div>`;
}

function initCardForm() {
  document.getElementById('bulletRows').innerHTML = [0, 1, 2].map(bulletRowHtml).join('');
}
...
async function loadCardOptions() {
  ...
  const themeLabels = { purple: '紫色系', blue: '藍色系', pink: '粉色系', green: '綠色系', neutral: '灰白系' };
  themeSelect.innerHTML = (data.themes || [])
    .map((t) => `<option value="${t}">${themeLabels[t] || t}</option>`)
    .join('');
}

function collectBullets() {
  const icons = document.querySelectorAll('[data-bullet-icon]');
  const bullets = [];
  icons.forEach((sel) => {
    const i = sel.dataset.bulletIcon;
    const textEl = document.querySelector(`[data-bullet-text="${i}"]`);
    const text = textEl ? textEl.value.trim() : '';
    if (text) bullets.push({ icon: sel.value, text });
  });
  return bullets;
}

document.getElementById('generateCardBtn').addEventListener('click', async () => {
  const fileInput = document.getElementById('cardProductPhoto');
  const msg = document.getElementById('cardMsg');
  const resultEl = document.getElementById('generateCardResult');
  const headline = document.getElementById('cardHeadline').value.trim();

  if (!fileInput.files[0]) {
    msg.textContent = '請先上傳商品圖片';
    msg.className = 'message error';
    return;
  }
  if (!headline) {
    msg.textContent = '請輸入大標題';
    msg.className = 'message error';
    return;
  }
  ...
  fd.append('productName', document.getElementById('cardProductName').value.trim());
  fd.append('tag', document.getElementById('cardTag').value.trim());
  fd.append('headline', headline);
  fd.append('subheadline', document.getElementById('cardSubheadline').value.trim());
  fd.append('theme', document.getElementById('cardTheme').value);
  fd.append('bulletsJson', JSON.stringify(collectBullets()));

  msg.className = 'message';
  resultEl.innerHTML = '<div class="generate-loading">排版生成中…</div>';

  try {
    const res = await fetch(`${API}/generate-card`, { method: 'POST', body: fd });
    const data = await res.json();
    if (!res.ok) {
      resultEl.innerHTML = '';
      msg.textContent = data.error || '生成失敗';
    ...
    resultEl.innerHTML = `<img src="${API}/generated/${data.item.id}/file" alt="${escapeHtml(headline)}" />`;
